...
{
  this.height0 = g_random.normal( params.height0, params.height0Var );
  this.height1 = g_random.normal( params.height1, params.height1Var );
  //this.heights=[h0,h1];
  this.totalHeight = this.height0 + this.height1;
  this.origin = createVector(xPos,0);
  this.seg0 = createVector(0,-this.height0);
  this.fixedBend = g_random.normal( params.bend, params.bendVar );
  ...
  {
    this.animFactors.push( random(1) );
  }

  this.verts = [];
  this.stepCount = int(this.totalHeight / 10);  // rule of thumb
  for ( var j = 0; j < this.stepCount * 2; j++ )
  {
    this.verts.push(createVector());
  }

  this.Animate = function(animParams)
  {
      var angle = 0;
      for(var i=0;i<animParams.length;i++)
      {
        angle += this.animFactors[i] * animParams[i].bendAngle;
      }

      this.seg1 = CreateDirVectorFromAngle(-HALF_PI + this.fixedBend + angle).mult(this.height1);
...
//    DrawBezier(this.origin,this.cp0,this.cp1,this.endPoint);
    ...
    // todo just draw verts! begin shape!
    {
      ...
      for ( var j = 0; j < steps; j++ )
      {
        ...
        this.verts[j].set(x - normalVector.x, y - normalVector.y);
        this.verts[(2*this.stepCount) - j - 1].set(x + normalVector.x, y + normalVector.y);
...
/*
      ...
      for(var i=0;i<this.stepCount*2;i++)
      {
