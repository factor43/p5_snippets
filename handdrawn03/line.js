...
function createVert( x, y, d )
{
  return createVector( x + randomAroundZero( d ), y + randomAroundZero( d ) );
}

var Rect = function( params, r )
{
  var d = params.radiusInc * 0.5;
  this.verts = [ createVert( -r, -r, d ), createVert( r, -r, d ), createVert( r, r, d ), createVert( -r, r, d ) ];
  this.lines = [];
  this.params = params;

  for ( var i = 0; i < 4; i++ )
  {
    this.lines.push( new HandDrawnLine( params, this.verts[ i ], this.verts[ ( i + 1 ) % 4 ] ) );
  }
