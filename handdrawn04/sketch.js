...
function initGui()
{
  ...
  GUI_ADD_SLIDER("lineCount",1,5000,RebuildAndRedraw);
  GUI_ADD_SLIDER("lineLength",50,800,RebuildAndRedraw);
  GUI_ADD_SLIDER("lineLengthVar",0,100,RebuildAndRedraw);
...
function RebuildAndRedraw()
{
  ...
  {
    var centre = createVector(g_canvasWidth / 2, g_canvasHeight / 2);

    var p2 = CreateDirVectorFromAngle(random(0,360));

    var len = g_random.normal( params.lineLength, params.lineLengthVar );

    p2.mult(len).add(centre);

    //var p2 = createVector(random(-params.lineLength,params.lineLength),);

  //  var xPos = random(0,g_canvasWidth);
  //  var p1 = createVector(xPos,0);
  //  var p2 = createVector(xPos + random(-5,5),params.lineLength);
    g_lines.push( new HandDrawnLine(params,centre,p2) );
  }
